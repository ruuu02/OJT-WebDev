<section
  ...
  class="menu-connect"
  @if($cv('contact_bg', '') !== '')
    style="--menu-connect-bg: {{ $cv('contact_bg', '') }};"
  @endif
>
  <div class="menu-connect-inner">
    <div class="menu-connect-head">
      <h2 class="menu-connect-title" data-ve-field="contact_heading">{{ $cv('contact_heading', 'CONNECT WITH US!') }}</h2>
      <p class="menu-connect-subtext" data-ve-field="contact_subtext">{{ $cv('contact_subtext', "Hungry for answers? Let's cook something up together - send us a bite of your thoughts!") }}</p>
    </div>

    <form
      id="menuConnectForm"
      class="menu-connect-form"
      method="POST"
    ...
    >
      @csrf
      <input type="hidden" name="page_slug" value="menu">
      <div class="menu-connect-grid">
        <div class="menu-connect-field">
          <label class="menu-connect-label" data-ve-field="contact_form_company_label">{{ $cv('contact_form_company_label', 'Company Name') }}</label>
          <input type="text" name="company" class="menu-connect-input" placeholder="{{ $cv('contact_form_company_ph', 'e.g. Acme Corp') }}" required>
        </div>

        <div class="menu-connect-field">
          <label class="menu-connect-label" data-ve-field="contact_form_industry_label">{{ $cv('contact_form_industry_label', 'Industry') }}</label>
          <input type="text" name="industry" class="menu-connect-input" placeholder="{{ $cv('contact_form_industry_ph', 'e.g. Food & Beverage') }}">
        </div>

        <div class="menu-connect-field">
          <label class="menu-connect-label" data-ve-field="contact_form_name_label">{{ $cv('contact_form_name_label', 'Full Name') }}</label>
          <input type="text" name="name" class="menu-connect-input" placeholder="{{ $cv('contact_form_name_ph', 'Your name') }}" required>
        </div>

        <div class="menu-connect-field">
          <label class="menu-connect-label" data-ve-field="contact_form_email_label">{{ $cv('contact_form_email_label', 'Email Address') }}</label>
          <input type="email" name="email" class="menu-connect-input" placeholder="{{ $cv('contact_form_email_ph', 'you@company.com') }}" required>
        </div>

        <div class="menu-connect-field menu-connect-field--full">
          <label class="menu-connect-label" data-ve-field="contact_form_message_label">{{ $cv('contact_form_message_label', 'Message') }}</label>
          <textarea name="message" class="menu-connect-input menu-connect-input--textarea" placeholder="{{ $cv('contact_form_message_ph', 'Tell us about your inquiry...') }}" required rows="4"></textarea>
        </div>
      </div>

      <div class="menu-connect-actions">
        <button type="submit" class="menu-connect-submit" data-ve-field="contact_form_submit">{{ $cv('contact_form_submit', 'Send Message') }} -></button>
      </div>
    </form>

    <div id="menuConnectSuccess" class="menu-connect-success" data-ve-field="contact_form_success">
      {{ $cv('contact_form_success', "Thank you! We'll be in touch soon.") }}
    </div>
    <div id="menuConnectError" class="menu-connect-error" role="alert">
...
<script>
  (function () {
    ...
    const form = document.getElementById('menuConnectForm');
    const success = document.getElementById('menuConnectSuccess');
    const error = document.getElementById('menuConnectError');
    ...
    form.addEventListener('submit', async function (event) {
      ...
      try {
        ...
        if (!formData.get('page_slug')) {
          formData.append('page_slug', form.dataset.pageSlug || 'menu');
        }
