...
const VISUAL_EDITOR_IMAGE_SOURCE_MAX_BYTES = 30 * 1024 * 1024;
// Keep raster uploads safely under PHP's multipart ceiling by compressing
// them before they hit the server.
const VISUAL_EDITOR_IMAGE_UPLOAD_TARGET_BYTES = 1750 * 1024;
const VISUAL_EDITOR_IMAGE_MAX_DIMENSION = 1600;
const VISUAL_EDITOR_IMAGE_ALLOWED_TYPES = ['image/jpeg', 'image/png', 'image/webp'];
const VISUAL_EDITOR_VIDEO_ALLOWED_TYPES = [
  'video/mp4',
  'video/webm',
  'video/ogg',
  'video/quicktime',
...
async function normalizeUploadImage(file, options = {}) {
  ...
  const {
    ...
    maxBytes = VISUAL_EDITOR_IMAGE_UPLOAD_TARGET_BYTES,
    maxSourceBytes = VISUAL_EDITOR_IMAGE_SOURCE_MAX_BYTES,
    quality = 0.82,
    minQuality = 0.55,
  ...
  if (file.size > maxSourceBytes) {
    throw new Error('The selected image is larger than 30 MB. Please choose a smaller image.');
  }
...
async function appendNormalizedImageToFormData(formData, fieldName, file, options = {}) {
  if (!(formData instanceof FormData) || !(file instanceof File)) return;
  formData.delete(fieldName);
  const rawType = String(file.type || '').toLowerCase();
  const isSvg = rawType === 'image/svg+xml' || /\.svg$/i.test(String(file.name || ''));
  const preserveOriginal = options?.preserveOriginal === true;
  if ((isSvg || preserveOriginal) && file.size > VISUAL_EDITOR_IMAGE_SOURCE_MAX_BYTES) {
    throw new Error('The selected image is larger than 30 MB. Please choose a smaller image.');
  }
  const normalizedFile = (isSvg || preserveOriginal) ? file : await normalizeUploadImage(file);
  if (normalizedFile) {
...
function initAccountDropdown() {
  const dropdown = document.getElementById('ve-account-dropdown');
  const menu = document.getElementById('ve-account-menu');
  const btn = document.getElementById('ve-account-btn');
  const chevron = document.getElementById('ve-account-chevron');
  if (!dropdown || !menu || !btn || !chevron) return;
  ...
  document.addEventListener('click', (e) => {
    if (!e.target.closest('#ve-account-dropdown')) {
      close();
    }
  });
...
function injectOverlay(el, zoneKey, zone) {
  ...
  if (zone?.overlay_mode === 'detached') {
    ...
    const detachedPosition = String(zone?.detached_position || '').trim();
    const placeDetachedButton = () => {
      if (detachedPosition === 've-bar-top-right') {
        btn.style.top = 'calc(var(--ve-bar-h) + 12px)';
      ...
      const height = btn.offsetHeight || 36;
      if (detachedPosition === 'center') {
        btn.style.top = `${Math.max(8, rect.top + (rect.height / 2) - (height / 2))}px`;
        btn.style.left = `${Math.max(8, rect.left + (rect.width / 2) - (width / 2))}px`;
    ...
    const alwaysVisible = zone?.overlay_always_visible === true;
    if (alwaysVisible) {
      requestAnimationFrame(showDetachedButton);
    } else {
...
function buildOverlays() {
  ...
  [...collected, ...mcliCollected].forEach(({ zoneKey, zone, el }) => {
    ...
    const clickableTarget = btn.closest('.ve-edit-zone, .ve-edit-target');
    if (zoneKey === 'menu_home_story_video' && clickableTarget && !clickableTarget.dataset.veStoryVideoBound) {
      clickableTarget.dataset.veStoryVideoBound = '1';
      clickableTarget.style.cursor = 'pointer';
      clickableTarget.addEventListener('click', (e) => {
        if (e.target?.closest?.('.ve-pencil-btn')) return;
        e.preventDefault();
        e.stopPropagation();
...
let _textOriginalMediaSizing = null; // { maxWidth, height, fit } when modal opened
...
function resolveTextZoneFields(zoneKey, zone) {
  ...
  const supportsDynamicFields =
    ...
    || (ACTIVE_PAGE === 'nordic_product_details' && zoneKey === 'nordic_product_details_selected_product')
    || (ACTIVE_PAGE === 'nordic' && ['nordic_why_section', 'nordic_oats_benefits'].includes(zoneKey));
  ...
  if (ACTIVE_PAGE === 'nordic_product_details' && zoneKey === 'nordic_product_details_selected_product') {
    const product = resolveNordicSelectedProductConfig(zone);
    if (!product) return configured;
    zone._nordicSelectedProduct = product;
    zone.defaults = { ...(zone.defaults || {}), ...(product.defaults || {}) };
    zone.field_labels = { ...(zone.field_labels || {}), ...(product.field_labels || {}) };
    return Array.isArray(product.fields) && product.fields.length ? product.fields : configured;
  }
  ...
  const dynamic = [];
  const maybeAddDynamicField = (fieldKey) => {
    if (!fieldKey) return;
    ...
    if (zoneKey === 'nordic_why_section' && (/^nordic_why_heading$/.test(fieldKey) || /^nordic_product_copy_(title|text)_\d+$/.test(fieldKey))) {
      dynamic.push(fieldKey);
    }
    if (zoneKey === 'nordic_oats_benefits' && (/^nordic_oats_benefits_heading$/.test(fieldKey) || /^nordic_oats_benefits_(copy_title|body)_\d+$/.test(fieldKey))) {
      dynamic.push(fieldKey);
    }
  };
  ...
  if (rootFieldKey) {
    maybeAddDynamicField(rootFieldKey);
  }
  root.querySelectorAll('[data-ve-field]').forEach((el) => {
    maybeAddDynamicField(el.getAttribute('data-ve-field'));
  });
...
function menuCategoryPageId(categorySlug) {
  ...
  const canonical = normalized === 'noodles-pastas-and-sauces'
    ? 'noodles-and-pastas'
    : normalized;
  const pageKey = canonical === 'noodles-and-pastas'
    ? 'menu_cat_noodles_pastas'
    : `menu_cat_${canonical.replace(/-/g, '_')}`;
...
function getElementEditorValue(el) {
  if (!el) return '';
...
function getLiveFieldValue(zone, fieldKey) {
  const canvas = document.getElementById('ve-canvas');
  if (!canvas || !fieldKey) return '';

  const directEl = canvas.querySelector(`[data-ve-field="${CSS.escape(fieldKey)}"]`);
  const directValue = getElementEditorValue(directEl);
  if (directValue) return directValue;

  const zoneRoot = zone?.selector ? canvas.querySelector(zone.selector) : canvas;
  if (!zoneRoot) return '';

  const contactPlaceholderMap = {
    contact_form_company_ph: 'company',
    contact_form_industry_ph: 'industry',
    contact_form_name_ph: 'name',
    contact_form_email_ph: 'email',
    contact_form_message_ph: 'message',
  };

  if (contactPlaceholderMap[fieldKey]) {
    const name = contactPlaceholderMap[fieldKey];
    const input = zoneRoot.querySelector(`[name="${CSS.escape(name)}"]`);
    return String(input?.getAttribute('placeholder') || '').trim();
  }

  if (fieldKey === 'contact_form_submit') {
    return getElementEditorValue(zoneRoot.querySelector('button[type="submit"], input[type="submit"]'));
  }

  if (fieldKey === 'contact_form_success') {
    return getElementEditorValue(zoneRoot.parentElement?.querySelector('#menuConnectSuccess, #sharedConnectSuccess, #formSuccess'));
  }
...
function appendMenuRecipeCategoryTitleTools(container, zone, fields) {
  if (_textZoneKey !== 'menu_recipe_category_title') return;

  const fieldKey = Array.isArray(fields) && fields.length === 1 ? String(fields[0] || '') : '';
  const match = fieldKey.match(/^menu_nav_recipe_category_(\d+)_label$/);
  if (!match) return;

  const categoryId = String(match[1] || '');
  const state = getVeRecipeState();
  const categories = Array.isArray(state?.categories) ? state.categories : [];
  const category = categories.find((item) => String(item?.id || '') === categoryId) || null;
  const canvas = document.getElementById('ve-canvas');
  const titleImg = canvas?.querySelector('.recipe-title-row img');
  const currentSrc = category?.icon_url || titleImg?.getAttribute('src') || '';

  const wrap = document.createElement('div');
  wrap.className = 've-field-group ve-menu-recipe-category-title-image';
  wrap.innerHTML = `
    <p class="ve-section-label" style="margin-top:1rem">Category icon</p>
    <div class="ve-mr-image-preview-wrap" style="margin-bottom:10px;max-width:220px;border-radius:10px;overflow:hidden;background:#f3f4f6;min-height:80px;display:flex;align-items:center;justify-content:center;">
      ${currentSrc
        ? `<img id="ve-menu-recipe-category-icon-preview" src="${esc(currentSrc)}" alt="" style="max-width:100%;max-height:160px;display:block;" />`
        : '<span class="ve-mr-no-image" style="padding:1rem;color:#888;font-size:13px;">No icon yet</span>'}
    </div>
    <input type="file" id="ve-menu-recipe-category-icon-input" accept="image/*,.svg" style="width:100%;font-size:13px;" />
    <p style="font-size:12px;color:#666;margin:6px 0 0">Choose a new file and Save to replace the category icon.</p>
  `;
  container.appendChild(wrap);

  const fileInp = wrap.querySelector('#ve-menu-recipe-category-icon-input');
  fileInp?.addEventListener('change', () => {
    const file = fileInp.files && fileInp.files[0];
    if (!file) return;
    let img = wrap.querySelector('#ve-menu-recipe-category-icon-preview');
    if (!img) {
      const empty = wrap.querySelector('.ve-mr-no-image');
      if (empty) empty.remove();
      const previewWrap = wrap.querySelector('.ve-mr-image-preview-wrap');
      if (previewWrap) {
        img = document.createElement('img');
        img.id = 've-menu-recipe-category-icon-preview';
        img.alt = '';
    ...
    if (img) {
      try {
        ...
        // Keep the current preview if the temporary file URL fails.
      }
    }
  });

  zone._menuRecipeCategoryId = categoryId;
}
...
function prependTextZoneMediaTools(container, zone) {
  ...
  const maxWidthKey = String(zone?.form_media_max_width_key || '').trim();
  const heightKey = String(zone?.form_media_height_key || '').trim();
  const fitKey = String(zone?.form_media_fit_key || '').trim();

  if (!maxWidthKey && !heightKey && !fitKey) {
    _textOriginalMediaSizing = null;
    return;
  }

  const maxWidthValue = maxWidthKey ? String(findContentRow(maxWidthKey, targetPageId)?.value || '').trim() : '';
  const heightValue = heightKey ? String(findContentRow(heightKey, targetPageId)?.value || '').trim() : '';
  const fitValue = fitKey ? String(findContentRow(fitKey, targetPageId)?.value || '').trim() : '';
  _textOriginalMediaSizing = { maxWidth: maxWidthValue, height: heightValue, fit: fitValue };

  const sizeWrap = document.createElement('div');
  sizeWrap.className = 've-field-group ve-text-zone-media-size-group';
  sizeWrap.dataset.maxWidthKey = maxWidthKey;
  sizeWrap.dataset.heightKey = heightKey;
  sizeWrap.dataset.fitKey = fitKey;
  sizeWrap.innerHTML = `
    <p class="ve-section-label" style="margin-top:1rem">Callout image size</p>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;align-items:end;">
      <div>
        <label style="display:block;font-size:12px;color:#334155;margin:0 0 6px;">Max width (px)</label>
        <input type="number" class="ve-input ve-text-zone-media-max-width" value="${esc(maxWidthValue)}" min="0" step="1" placeholder="e.g. 520" />
      </div>
      <div>
        <label style="display:block;font-size:12px;color:#334155;margin:0 0 6px;">Height (px)</label>
        <input type="number" class="ve-input ve-text-zone-media-height" value="${esc(heightValue)}" min="0" step="1" placeholder="e.g. 50" />
      </div>
    </div>
    <div style="margin-top:10px;">
      <label style="display:block;font-size:12px;color:#334155;margin:0 0 6px;">Fit</label>
      <select class="ve-select ve-text-zone-media-fit" style="width:100%;">
        <option value=""${fitValue === '' ? ' selected' : ''}>Default (contain)</option>
        <option value="contain"${fitValue === 'contain' ? ' selected' : ''}>Contain</option>
        <option value="cover"${fitValue === 'cover' ? ' selected' : ''}>Cover</option>
      </select>
    </div>
    <p style="font-size:12px;color:#666;margin:8px 0 0">Leave blank to use the default CSS sizing.</p>
  `;
  wrap.insertAdjacentElement('afterend', sizeWrap);
}
...
function isNordicSelectedProductZone(zoneKey) {
  return zoneKey === 'nordic_products_selected_product'
    || zoneKey === 'nordic_product_details_selected_product';
}

function resolveNordicProductMediaTools(zone) {
  const product = zone?._nordicSelectedProduct || resolveNordicSelectedProductConfig(zone);
  if (!product) return [];
  if (Array.isArray(product.media_tools) && product.media_tools.length) {
    return product.media_tools.filter((tool) => tool?.media_key);
  }
  const fallbackTools = [
    { label: product.front_label || 'Front image', media_key: product.front_media_key, fallback: product.front_fallback },
    { label: product.back_label || 'Back image', media_key: product.back_media_key, fallback: product.back_fallback },
  ];
  return fallbackTools.filter((tool) => tool?.media_key);
}

function appendNordicSelectedProductTools(container, zone) {
  if (!isNordicSelectedProductZone(_textZoneKey)) return;
  ...
  const targetPageId = zonePageId(_textZoneKey);
  zone._nordicSelectedProduct = product;
  zone._nordicMediaTools = resolveNordicProductMediaTools(zone);
  ...
  const imageTools = zone._nordicMediaTools || [];

  imageTools.slice().reverse().forEach((tool) => {
    const mediaKey = String(tool.media_key || '').trim();
    const previewSrc = buildPreviewSrc(mediaKey, tool.fallback);
    const wrap = document.createElement('div');
    wrap.className = 've-field-group ve-nordic-product-media-group';
    wrap.dataset.mediaKey = mediaKey;
    wrap.innerHTML = `
      <label>${esc(tool.label || 'Product image')}</label>
...
function isVideoUploadZone(zone) {
  return Boolean(zone?.video_upload_enabled && Array.isArray(zone?.fields) && zone.fields.length);
}

function getVideoUploadFieldKey(zone) {
  return isVideoUploadZone(zone) ? String(zone.fields[0] || '').trim() : '';
}
...
function extractYouTubeVideoId(value) {
  ...
  const shortMatch = src.match(/(?:youtube\.com\/shorts\/|youtu\.be\/)([a-zA-Z0-9_-]{6,})/i);
...
function resolveVideoPreviewSource(value) {
  const src = String(value || '').trim();
  if (!src) return { kind: 'empty', src: '' };
  ...
  if (src.startsWith('blob:')) return { kind: 'file', src };
  if (/^https?:\/\//i.test(src) || src.startsWith('//')) return { kind: 'file', src };
  if (src.startsWith('/')) return { kind: 'file', src };
  return { kind: 'file', src: `/${src.replace(/^\/+/, '')}` };
}

function renderVideoPreviewMarkup(value, emptyText = 'No video selected yet.') {
  const preview = resolveVideoPreviewSource(value);
  if (preview.kind === 'youtube') {
    return `<iframe
      class="ve-text-zone-video-element"
      src="${esc(preview.src)}"
      title="Video preview"
      loading="lazy"
      ...
      style="width:100%;aspect-ratio:16 / 9;display:block;border:0;background:#000;"
    ></iframe>`;
  }

  if (preview.kind === 'file' && preview.src) {
    return `<video class="ve-text-zone-video-element" src="${esc(preview.src)}" controls playsinline preload="metadata" style="width:100%;max-height:240px;display:block;background:#000;"></video>`;
  }

  return `<span class="ve-text-zone-video-empty" style="display:block;padding:18px;color:#64748b;font-size:13px;">${esc(emptyText)}</span>`;
}

function appendTextZoneVideoTools(container, zone) {
  const fieldKey = getVideoUploadFieldKey(zone);
  if (!fieldKey) return;

  const input = container.querySelector(`[data-key="${CSS.escape(fieldKey)}"]`);
  const group = input?.closest('.ve-field-group');
  if (!group || container.querySelector('.ve-text-zone-video-group')) return;

  const currentValue = String(input?.value || '').trim();
  const wrap = document.createElement('div');
  wrap.className = 've-field-group ve-text-zone-video-group';
  wrap.innerHTML = `
    <label>${esc(zone.video_help_text || 'Upload a video or paste a direct link.')}</label>
    <div class="ve-text-zone-video-preview" style="border:1px solid #d7dfec;border-radius:14px;overflow:hidden;background:#f4f7fb;margin-bottom:10px;">
      ${renderVideoPreviewMarkup(currentValue)}
    </div>
    <input type="file" class="ve-text-zone-video-file" accept="video/mp4,video/webm,video/ogg,video/quicktime,video/x-m4v,.mp4,.webm,.ogg,.mov,.m4v" style="width:100%;font-size:13px;" />
    <p style="font-size:12px;color:#666;margin:6px 0 0">Upload an MP4, WEBM, OGG, MOV, or M4V file. Direct file links and YouTube links are supported.</p>
  `;

  group.appendChild(wrap);

  const preview = wrap.querySelector('.ve-text-zone-video-preview');
  const fileInput = wrap.querySelector('.ve-text-zone-video-file');
  const renderPreview = (src) => {
    if (!preview) return;
    preview.innerHTML = renderVideoPreviewMarkup(src);
  };

  wrap._renderVideoPreview = renderPreview;
  ...
  fileInput?.addEventListener('change', () => {
    ...
    if (file.type && !VISUAL_EDITOR_VIDEO_ALLOWED_TYPES.includes(file.type)) {
      fileInput.value = '';
      toast('Please choose an MP4, WEBM, OGG, MOV, or M4V video.', 'error');
      renderPreview(input?.value || '');
      return;
    }
    ...
    try {
      ...
      toast('Could not preview the selected video.', 'error');
    }
  });
}

async function uploadTextZoneVideoFile(file, fieldKey, targetPageId, zone) {
  if (!file || !fieldKey || !targetPageId) {
    return { ok: false, message: 'Missing video upload details.' };
  }

  if (file.type && !VISUAL_EDITOR_VIDEO_ALLOWED_TYPES.includes(file.type)) {
    return { ok: false, message: 'Please choose an MP4, WEBM, OGG, MOV, or M4V video.' };
  }

  const form = new FormData();
  form.append('video', file);
  form.append('page_id', targetPageId);
  form.append('base_key', fieldKey);
  form.append('directory', String(zone?.video_upload_directory || 'menu'));

  const { ok, data } = await apiFetch(ROUTES.contentUploadVideoField, 'POST', form);
  if (!ok) {
    return { ok: false, message: apiErrorMessage(data, 'Could not upload the video.') };
  }

  const nextValue = String(data?.path || '').trim();
  if (!nextValue) {
    return { ok: false, message: 'Video upload finished, but no saved path was returned.' };
  }

  const cacheIndex = ALL_CONTENTS.findIndex((row) =>
    row.key === fieldKey && Number(row.page_id || 0) === Number(targetPageId)
  );
  if (cacheIndex >= 0) {
    ALL_CONTENTS[cacheIndex].value = nextValue;
  } else {
...
function organizeNordicProductDetailsFields(container, zone) {
  if (_textZoneKey !== 'nordic_product_details_selected_product') return;

  const product = zone?._nordicSelectedProduct || resolveNordicSelectedProductConfig(zone);
  if (!product) return;

  const fieldGroups = collectMenuHeaderFieldGroups(container);
  const byFieldKey = new Map();
  fieldGroups.forEach((group) => {
    const key = fieldKeyFromGroup(group);
    if (key) byFieldKey.set(key, group);
  });

  const mediaGroups = new Map();
  container.querySelectorAll('.ve-nordic-product-media-group').forEach((group) => {
    const mediaKey = String(group?.dataset?.mediaKey || '').trim();
    if (mediaKey) mediaGroups.set(mediaKey, group);
  });

  const appendField = (target, key) => {
    const group = byFieldKey.get(String(key || ''));
    if (group) target.appendChild(group);
  };

  const appendMedia = (target, mediaKey) => {
    const group = mediaGroups.get(String(mediaKey || ''));
    if (group) target.appendChild(group);
  };

  const generalSection = createMenuHeaderNavSection(
    'General',
    'Main content shared across all sizes',
    true,
  );
  const oneKgSection = createMenuHeaderNavSection(
    '1kg',
    'Label, links, and images for the 1kg variant',
    false,
  );
  const fiveHundredSection = createMenuHeaderNavSection(
    '500g',
    'Label, links, and images for the 500g variant',
    false,
  );
  const twoFiftySection = createMenuHeaderNavSection(
    '250g',
    'Label, links, and images for the 250g variant',
    false,
  );

  const generalBody = generalSection.querySelector('.ve-nav-modal-section-body');
  const oneKgBody = oneKgSection.querySelector('.ve-nav-modal-section-body');
  const fiveHundredBody = fiveHundredSection.querySelector('.ve-nav-modal-section-body');
  const twoFiftyBody = twoFiftySection.querySelector('.ve-nav-modal-section-body');

  appendField(generalBody, product.title_key);
  appendField(generalBody, product.description_key);
  appendField(generalBody, product.size_title_key);
  appendField(generalBody, product.feature_1_key);
  appendField(generalBody, product.feature_2_key);
  appendField(generalBody, product.feature_3_key);
  appendField(generalBody, product.buy_title_key);
  appendField(generalBody, product.buy_shopee_label_key);
  appendField(generalBody, product.buy_lazada_label_key);
  appendField(generalBody, product.buy_tiktok_label_key);

  appendField(oneKgBody, product.size_1kg_label_key);
  appendField(oneKgBody, product.size_1kg_shopee_key);
  appendField(oneKgBody, product.size_1kg_lazada_key);
  appendField(oneKgBody, product.size_1kg_tiktok_key);
  appendMedia(oneKgBody, product.size_1kg_front_media_key);
  appendMedia(oneKgBody, product.size_1kg_back_media_key);

  appendField(fiveHundredBody, product.size_500g_label_key);
  appendField(fiveHundredBody, product.size_500g_shopee_key);
  appendField(fiveHundredBody, product.size_500g_lazada_key);
  appendField(fiveHundredBody, product.size_500g_tiktok_key);
  appendMedia(fiveHundredBody, product.size_500g_front_media_key);
  appendMedia(fiveHundredBody, product.size_500g_back_media_key);

  appendField(twoFiftyBody, product.size_250g_label_key);
  appendField(twoFiftyBody, product.size_250g_shopee_key);
  appendField(twoFiftyBody, product.size_250g_lazada_key);
  appendField(twoFiftyBody, product.size_250g_tiktok_key);
  appendMedia(twoFiftyBody, product.size_250g_front_media_key);
  appendMedia(twoFiftyBody, product.size_250g_back_media_key);

  const leftovers = Array.from(container.children).filter((child) =>
    child.classList.contains('ve-field-group') || child.classList.contains('ve-nordic-product-media-group')
  );
  leftovers.forEach((node) => generalBody?.appendChild(node));

  container.innerHTML = '';
  [generalSection, oneKgSection, fiveHundredSection, twoFiftySection].forEach((section) => {
    const body = section.querySelector('.ve-nav-modal-section-body');
    if (!body || !body.children.length) return;
    container.appendChild(section);
  });
...
function openTextModal(zoneKey, zone) {
  ...
  _textZone    = zone;
  zone._menuRecipeCategoryId = null;
  ...
  fields.forEach(fieldKey => {
    ...
    const storedValue = existing ? (existing.value ?? '') : '';
    const liveValue = getLiveFieldValue(zone, fieldKey);
    const value = String(storedValue || '').trim() !== ''
      ? storedValue
      : (liveValue || zone.defaults?.[fieldKey] || '');
    ...
    const isVideoField = isVideoUploadZone(zone) && getVideoUploadFieldKey(zone) === fieldKey;
    ...
    group.innerHTML = `
      ...
      ${isLong
        ? `<textarea id="ve-field-${esc(fieldKey)}" class="ve-textarea" data-key="${esc(fieldKey)}" rows="3" ${isVideoField ? `placeholder="${esc(zone?.video_input_placeholder || '')}"` : ''}>${esc(value)}</textarea>`
        : `<input type="text" id="ve-field-${esc(fieldKey)}" class="ve-input" data-key="${esc(fieldKey)}" value="${esc(value)}" ${isVideoField ? `placeholder="${esc(zone?.video_input_placeholder || '')}"` : ''} />`
      }
      ${isVideoField ? '' : `<div class="ve-field-style" data-field-key="${esc(fieldKey)}">
        <button type="button" class="ve-field-style-toggle" aria-expanded="false">Style options</button>
        <div class="ve-field-style-body" hidden>
          <div class="ve-style-row">
  ...
  appendTextZoneVideoTools(container, zone);
  appendMenuHeaderProductTools(container, zone);
  appendMenuHeaderRecipeCategoryTools(container);
  appendMenuRecipeCategoryTitleTools(container, zone, fields);
  ...
  organizeNordicProductDetailsFields(container, zone);
...
function snapshotTextZoneMediaState(zone, targetPageId) {
  ...
  return {
    ...
    maxWidth: zone?.form_media_max_width_key
      ? String(findContentRow(zone.form_media_max_width_key, targetPageId)?.value || '').trim()
      : '',
    height: zone?.form_media_height_key
      ? String(findContentRow(zone.form_media_height_key, targetPageId)?.value || '').trim()
      : '',
    fit: zone?.form_media_fit_key
      ? String(findContentRow(zone.form_media_fit_key, targetPageId)?.value || '').trim()
      : '',
  };
}

function snapshotNordicSelectedProductState(zone, targetPageId) {
  const mediaTools = resolveNordicProductMediaTools(zone);
  const mediaValues = {};
  mediaTools.forEach((tool) => {
    const key = String(tool?.media_key || '').trim();
    if (!key) return;
    mediaValues[key] = String(findContentRow(key, targetPageId)?.value || '').trim();
  });
  return { mediaValues };
}
...
async function uploadMenuHeaderIcon(file, targetPageId) {
  ...
  await appendNormalizedImageToFormData(fd, 'image', file);
...
async function saveTextModal() {
  ...
  } else if (isNordicSelectedProductZone(_textZoneKey)) {
  ...
  if (isVideoUploadZone(_textZone)) {
    const videoFieldKey = getVideoUploadFieldKey(_textZone);
    const videoInput = document.querySelector(`[data-key="${CSS.escape(videoFieldKey)}"]`);
    const videoFile = document.querySelector('.ve-text-zone-video-file')?.files?.[0] || null;

    if (videoFile) {
      const uploadResult = await uploadTextZoneVideoFile(videoFile, videoFieldKey, targetPageId, _textZone);
      if (!uploadResult.ok) {
        allOk = false;
        partialSave = true;
        saveIssues.push(uploadResult.message || 'Could not upload the selected video.');
      } else if (videoInput) {
        videoInput.value = uploadResult.path;
      }
    }
  }
  ...
  const categoryId = String(_textZone?._menuRecipeCategoryId || '').trim();
  const categoryFieldKey = fields.length === 1 ? String(fields[0] || '') : '';
  const categoryNameInput = categoryFieldKey
    ? document.querySelector(`[data-key="${categoryFieldKey}"]`)
    : null;
  const categoryIconInput = document.getElementById('ve-menu-recipe-category-icon-input');
  const categoryIconFile = categoryIconInput?.files?.[0] || null;
  if (categoryId && (categoryNameInput || categoryIconFile)) {
    const categoryName = String(categoryNameInput?.value || '').trim();
    const { ok, data } = await saveInlineRecipeCategory(categoryId, categoryName, categoryIconFile);
    if (!ok || !data?.ok || !data?.category) {
      allOk = false;
      partialSave = true;
      saveIssues.push(apiErrorMessage(data, 'Could not save the recipe category details.'));
      console.error('recipe category title save failed', data);
    } else {
      ...
      const existing = categories.find((item) => String(item?.id || '') === String(nextCategory.id || ''));
      if (existing) {
        Object.assign(existing, nextCategory);
      } else {
        categories.push(nextCategory);
      }
      if (currentState && String(currentState?.activeCategory?.id || '') === String(nextCategory.id || '')) {
        currentState.activeCategory = { ...currentState.activeCategory, ...nextCategory };
      }

      const canvas = document.getElementById('ve-canvas');
      const titleImg = canvas?.querySelector('.recipe-title-row img');
      if (titleImg && nextCategory.icon_url) {
        titleImg.src = nextCategory.icon_url;
      }
    }
  }
  ...
  if (allOk && _textZone?.form_media_key && _textZone?.form_media_section) {
    ...
    if (mediaFile) {
      ...
      form.append('page_id', targetPageId);
      await appendNormalizedImageToFormData(form, 'image', mediaFile);
  ...
  if (allOk && _textZone?.form_media_key) {
    const sizeGroup = document.querySelector('.ve-text-zone-media-size-group');
    if (sizeGroup) {
      const maxWidthKey = String(sizeGroup.dataset.maxWidthKey || '').trim();
      const heightKey = String(sizeGroup.dataset.heightKey || '').trim();
      const fitKey = String(sizeGroup.dataset.fitKey || '').trim();

      const maybeSaveKey = async (key, value, isDefault = false) => {
        if (!key) return;
        const existing = String(findContentRow(key, targetPageId)?.value || '').trim();
        const next = String(value || '').trim();
        const shouldRemove = next === '' || isDefault;

        if (shouldRemove && existing === '') return;
        if (!shouldRemove && existing === next) return;

        if (shouldRemove) {
          const result = await apiFetch(ROUTES.contentDestroyByKey, 'DELETE', {
            page_id: targetPageId,
            section: 'text_content',
            key,
          });
          if (!result.ok) {
            allOk = false;
            saveIssues.push(apiErrorMessage(result.data, `Could not clear ${key}.`));
            return;
          }
          const ci = ALL_CONTENTS.findIndex((row) => row.key === key && Number(row.page_id || 0) === targetPageId);
        ...
        if (!result.ok) {
          allOk = false;
          saveIssues.push(apiErrorMessage(result.data, `Could not save ${key}.`));
          return;
        }
        const ci = ALL_CONTENTS.findIndex((row) => row.key === key && Number(row.page_id || 0) === targetPageId);
        if (ci >= 0) ALL_CONTENTS[ci].value = next;
        else ALL_CONTENTS.push({ key, value: next, section: 'text_content', page_id: targetPageId });
      };

      const maxWidthVal = sizeGroup.querySelector('.ve-text-zone-media-max-width')?.value || '';
      const heightVal = sizeGroup.querySelector('.ve-text-zone-media-height')?.value || '';
      const fitVal = sizeGroup.querySelector('.ve-text-zone-media-fit')?.value || '';

      await maybeSaveKey(maxWidthKey, maxWidthVal, false);
      await maybeSaveKey(heightKey, heightVal, false);
      await maybeSaveKey(fitKey, fitVal, fitVal === '' || fitVal === 'contain');
    }
  }

  if (allOk && isNordicSelectedProductZone(_textZoneKey) && _textZone?._nordicSelectedProduct) {
    ...
    for (const group of mediaGroups) {
      ...
      form.append('page_id', targetPageId);
      await appendNormalizedImageToFormData(form, 'image', file);
...
async function undoTextChanges() {
  if (_textPrevSave) {
    ...
    if (isVideoUploadZone(_textZone)) {
      const videoFieldKey = getVideoUploadFieldKey(_textZone);
      const videoInput = document.querySelector(`[data-key="${CSS.escape(videoFieldKey)}"]`);
      const videoGroup = document.querySelector('.ve-text-zone-video-group');
      const videoFileInput = videoGroup?.querySelector('.ve-text-zone-video-file');
      if (videoInput) videoInput.value = _textOriginalVals[videoFieldKey] ?? '';
      if (videoFileInput) videoFileInput.value = '';
      if (typeof videoGroup?._renderVideoPreview === 'function') {
        videoGroup._renderVideoPreview(videoInput?.value || '');
      }
    }
    ...
    if (_textOriginalMediaSizing) {
      const sizeGroup = document.querySelector('.ve-text-zone-media-size-group');
      if (sizeGroup) {
        const maxWidthEl = sizeGroup.querySelector('.ve-text-zone-media-max-width');
        const heightEl = sizeGroup.querySelector('.ve-text-zone-media-height');
        const fitEl = sizeGroup.querySelector('.ve-text-zone-media-fit');
        if (maxWidthEl) maxWidthEl.value = String(_textOriginalMediaSizing.maxWidth || '').trim();
        if (heightEl) heightEl.value = String(_textOriginalMediaSizing.height || '').trim();
      if (fitEl) fitEl.value = String(_textOriginalMediaSizing.fit || '').trim();
      }
    }
...
async function performTextUndoLastSave() {
  ...
  if (allOk && _textZone?.form_media_key && _textPrevSave.__text_zone_media) {
    const snap = _textPrevSave.__text_zone_media;
    const maxWidthKey = String(_textZone?.form_media_max_width_key || '').trim();
    const heightKey = String(_textZone?.form_media_height_key || '').trim();
    const fitKey = String(_textZone?.form_media_fit_key || '').trim();

    const restoreKey = async (key, value, isDefault = false) => {
      if (!key) return true;
      const v = String(value || '').trim();
      const shouldRemove = v === '' || isDefault;
      ...
      if (!result.ok) {
        console.error('text zone media sizing undo failed', { key, v, data: result.data });
        return false;
      }
      const ci = ALL_CONTENTS.findIndex((row) => row.key === key && Number(row.page_id || 0) === targetPageId);
      if (ci >= 0) ALL_CONTENTS[ci].value = v;
      else ALL_CONTENTS.push({ key, value: v, section: 'text_content', page_id: targetPageId });
      return true;
    };

    const ok1 = await restoreKey(maxWidthKey, snap.maxWidth || '', false);
    const ok2 = await restoreKey(heightKey, snap.height || '', false);
    const fitVal = String(snap.fit || '').trim();
    const ok3 = await restoreKey(fitKey, fitVal, fitVal === '' || fitVal === 'contain');
    ...
    if (sizeGroup) {
      ...
      const heightEl = sizeGroup.querySelector('.ve-text-zone-media-height');
      const fitEl = sizeGroup.querySelector('.ve-text-zone-media-fit');
      if (maxWidthEl) maxWidthEl.value = String(snap.maxWidth || '').trim();
      if (heightEl) heightEl.value = String(snap.height || '').trim();
      if (fitEl) fitEl.value = String(snap.fit || '').trim();
    }
  }

  if (allOk && isNordicSelectedProductZone(_textZoneKey) && _textPrevSave.__nordic_selected_product) {
    const snap = _textPrevSave.__nordic_selected_product;
    const mediaPairs = Object.entries(snap?.mediaValues || {}).map(([key, value]) => ({
      key: String(key || '').trim(),
...
function resolveStyleCssVar(key, zoneKey = '') {
  if (key === 'contact_bg' && zoneKey === 'menu_connect_bg_style') {
    return '--menu-connect-bg';
  }
  if (key === 'contact_bg' && zoneKey === 'nordic_connect_bg_style') {
    return '--shared-connect-bg';
  }
  return STYLE_CSS_VAR_MAP[key] || null;
}
...
function applyStyleLivePreview(key, value) {
  const cssVar = resolveStyleCssVar(key, _styleZoneKey);
  if (_styleTargetEl && cssVar) _styleTargetEl.style.setProperty(cssVar, value);
}

function getLiveStyleValue(key, zoneKey, targetEl) {
  if (!targetEl) return '';
  const cssVar = resolveStyleCssVar(key, zoneKey);
  if (cssVar) {
    const inlineVar = targetEl.style.getPropertyValue(cssVar).trim();
    if (inlineVar) return inlineVar;
    const computedVar = getComputedStyle(targetEl).getPropertyValue(cssVar).trim();
...
function openStyleModal(zoneKey, zone) {
  ...
  const DEFAULTS = {
    ...
    clients_bg:           '#ffffff',
    contact_bg:           zoneKey === 'nordic_connect_bg_style' ? '#223947' : '#79CE1B',
    footer_bg:            '#ffffff',
  };

  fields.forEach(fieldKey => {
    const existing = findContentRow(fieldKey, targetPageId)?.value;
    const liveValue = getLiveStyleValue(fieldKey, zoneKey, _styleTargetEl);
    const raw = (existing != null && existing !== '')
      ? existing
      : (liveValue || DEFAULTS[fieldKey] || '#000000');
...
async function saveStyleModal() {
  ...
  const targetPageId = zonePageId(_styleZoneKey);
  ...
  for (const fieldKey of fields) {
    ...
    if (!ok) {
      ...
      if (idx >= 0) {
        ...
        ALL_CONTENTS.push({ key: fieldKey, value, section: 'text_content', page_id: targetPageId });
      }
...
function activeImageZoneSchema() {
  const pageSchema = CMS_SCHEMA[ACTIVE_PAGE];
  return pageSchema?.zones?.[_imageZoneKey] || null;
}

function preserveOriginalImageUpload() {
  return activeImageZoneSchema()?.preserve_original_upload === true;
}

function appendImageModalFile(formData, fieldName, file) {
  return appendNormalizedImageToFormData(formData, fieldName, file, {
    preserveOriginal: preserveOriginalImageUpload(),
  });
}

function updateImageModalUploadHint() {
  const hint = document.getElementById('ve-drop-zone-hint');
  if (!hint) return;
  hint.textContent = preserveOriginalImageUpload()
    ? 'JPG, PNG, WEBP or SVG - original file preserved, up to 30 MB'
    : 'JPG, PNG, WEBP or SVG - up to 30 MB';
}

function imageDimensionFields() {
  const fields = activeImageZoneSchema()?.dimension_fields || null;
  if (!fields || typeof fields !== 'object') return null;
  const width = String(fields.width || '').trim();
  const height = String(fields.height || '').trim();
...
function imageDimensionValue(value) {
  ...
  if (!raw) return '';
  return /^(auto|[0-9]+(?:\.[0-9]+)?(?:px|rem|em|vw|vh|%)|calc\([0-9a-zA-Z.%+\-*\/\s]+\))$/i.test(raw)
    ? raw
    : '';
}

function imageDimensionTarget() {
  const canvas = document.getElementById('ve-canvas');
  const zoneSchema = activeImageZoneSchema();
  if (!canvas || !zoneSchema?.selector) return null;
...
function applyImageDimensionsToCanvas(widthValue, heightValue) {
  const imgEl = imageDimensionTarget();
  if (!imgEl) return;
  const width = imageDimensionValue(widthValue);
  const height = imageDimensionValue(heightValue);
...
function updateImageDimensionControls() {
  const section = document.getElementById('ve-image-dimensions-section');
  if (!section) return;
  const fields = imageDimensionFields();
  section.hidden = !fields;
  if (!fields) return;

  const targetPageId = zonePageId(_imageZoneKey);
  const widthInput = document.getElementById('ve-image-width-input');
  const heightInput = document.getElementById('ve-image-height-input');
  const imgEl = imageDimensionTarget();
  const rect = imgEl?.getBoundingClientRect?.();
  const renderedWidth = rect?.width ? `${Math.round(rect.width)}px` : '';
  const renderedHeight = rect?.height ? `${Math.round(rect.height)}px` : '';
  const savedWidth = fields.width ? String(findContentRow(fields.width, targetPageId)?.value || '').trim() : '';
  const savedHeight = fields.height ? String(findContentRow(fields.height, targetPageId)?.value || '').trim() : '';

  if (widthInput) {
    widthInput.value = savedWidth || renderedWidth;
    widthInput.placeholder = renderedWidth ? `Current: ${renderedWidth}` : 'Default, e.g. 920px or 65vw';
  }
  if (heightInput) {
    heightInput.value = savedHeight || renderedHeight;
    heightInput.placeholder = renderedHeight ? `Current: ${renderedHeight}` : 'Default, e.g. 260px or auto';
  }
}
...
async function saveImageDimensions() {
  ...
  const saveBtn = document.getElementById('ve-image-dimensions-save');
  const widthInput = document.getElementById('ve-image-width-input');
  const heightInput = document.getElementById('ve-image-height-input');
  const widthValue = String(widthInput?.value || '').trim();
  const heightValue = String(heightInput?.value || '').trim();

  if (widthValue && !imageDimensionValue(widthValue)) {
    toast('Width must be a CSS size like 920px, 65vw, 100%, calc(100% - 20px), auto, or blank.', 'error');
    return;
  }
  if (heightValue && !imageDimensionValue(heightValue)) {
    toast('Height must be a CSS size like 260px, 30vh, auto, or blank.', 'error');
    return;
  }
  ...
  const targetPageId = zonePageId(_imageZoneKey);
  const updates = [
    fields.width ? [fields.width, widthValue] : null,
    fields.height ? [fields.height, heightValue] : null,
  ].filter(Boolean);
  ...
  for (const [baseKey, value] of updates) {
    ...
    const idx = ALL_CONTENTS.findIndex(r => r.key === baseKey && Number(r.page_id || 0) === targetPageId);
    if (idx >= 0) ALL_CONTENTS[idx].value = value;
    else ALL_CONTENTS.push({ key: baseKey, value, section: 'text_content', page_id: targetPageId });
  }

  if (saveBtn) {
    saveBtn.disabled = false;
    saveBtn.innerHTML = 'Save Size';
  }

  if (!allOk) {
    toast('Could not save the floater size.', 'error');
    return;
  }

  applyImageDimensionsToCanvas(widthValue, heightValue);
  toast('Floater size saved.');
  await refreshCanvas();
}
...
function openImageModal(zoneKey, zone) {
  ...
  updateImageModalUploadHint();
  updateImageDimensionControls();
...
function imageGridCustomRows(section) {
  ...
  const rows = zoneSchema.preview_selectors.map((selector, idx) => {
    ...
    return {
      ...
      replace_disabled: !(managedRow?.id || liveKeys[idx]),
    };
  }).filter(Boolean);
...
function imageGridConfig(section) {
  ...
  if (zoneSchema?.media_key) {
    ...
    const target     = targetId
      ...
      : null;
    if (target) {
      return { rows: [target], replaceOnly: true, customRows: false };
    }

    const canvas = document.getElementById('ve-canvas');
    const targetEl = canvas?.querySelector(zoneSchema.selector);
    const imgEl = targetEl?.tagName === 'IMG'
      ? targetEl
      : targetEl?.querySelector?.('img');
    ...
    if (imgEl?.src) {
      return {
        rows: [{
          ...
          section,
          is_active: true,
          original_name: zoneSchema.label || 'Current image',
          filename: '',
          preview_src: imgEl.src,
          live_key: zoneSchema.media_key,
    ...
    return { rows: [], replaceOnly: true, customRows: false };
  }
...
function renderImageGrid(section) {
  ...
  grid.innerHTML = rows.map(row => `
    <div class="ve-image-thumb ${row.is_active ? 've-image-thumb--active' : ''}"
      ...
      <div class="ve-image-thumb-actions">
        ...
        ${allowDelete && String(row.id || '').trim() !== ''
  ...
  if (!replaceInput) {
    ...
    replaceInput.accept = 'image/jpeg,image/png,image/webp,image/svg+xml,.svg';
  ...
  grid.querySelectorAll('.ve-image-replace-btn').forEach(btn => {
    btn.addEventListener('click', e => {
      e.stopPropagation();
      if (btn.disabled || (!btn.dataset.id && !btn.dataset.liveKey)) return;
  ...
  replaceInput.onchange = () => {
    ...
    const targetId = String(replaceInput.dataset.targetId || '').trim();
    const targetSection = replaceInput.dataset.targetSection;
    const targetLiveKey = replaceInput.dataset.targetLiveKey || '';

    if (targetId) {
      replaceImage(targetId, targetSection, file, targetLiveKey);
      return;
    }

    if (targetLiveKey) {
      uploadImageForLiveKey(targetSection, file, targetLiveKey);
    }
  };
...
async function replaceImage(id, section, file, liveKey = '') {
  ...
  try {
    await appendImageModalFile(form, 'image', file);
  } catch (e) {
    toast(e?.message || 'The selected image could not be prepared.', 'error');
...
async function uploadImageForLiveKey(section, file, liveKey) {
  if (!section || !file || !liveKey) return;
  ...
  const newRow = { ...data.media, section, is_active: true };
  ...
  if (ck) {
    ...
    else ALL_CONTENTS.push({ key: liveKey, value: String(newRow.id), section: 'text_content', page_id: targetPageId });
  }
  ...
  if (data.media?.filename) {
    ...
    const zoneSchema = pageSchema?.zones?.[_imageZoneKey];
    const canvas = document.getElementById('ve-canvas');
    const previewIdx = Array.isArray(zoneSchema?.media_keys)
      ? zoneSchema.media_keys.findIndex((key) => String(key) === String(liveKey))
      : -1;

    if (previewIdx >= 0 && Array.isArray(zoneSchema?.preview_selectors)) {
      const imgEl = canvas?.querySelector(zoneSchema.preview_selectors[previewIdx]);
      if (imgEl) {
        const dirMap = { carousel: 'banner', logo: 'logo', product: 'banner', recipe: 'icons' };
        const dir = dirMap[section] ?? section;
        imgEl.src = `/images/${dir}/${data.media.filename}`;
      }
    } else if (zoneSchema?.selector) {
      const targetEl = canvas?.querySelector(zoneSchema.selector);
      const imgEl = targetEl?.tagName === 'IMG'
        ? targetEl
        : targetEl?.querySelector?.('img');
...
async function uploadImage() {
  ...
  form.append('page_id', targetPageId);
  try {
    await appendImageModalFile(form, 'image', _pendingFile);
  } catch (e) {
    toast(e?.message || 'The selected image could not be prepared.', 'error');
...
function initDropZone() {
  ...
  zone.addEventListener('drop', e => {
    ...
    const isSupportedImage = /^image\/(jpeg|png|webp|svg\+xml)$/i.test(file?.type || '')
      || /\.svg$/i.test(String(file?.name || ''));
    if (file && isSupportedImage) previewFile(file);
    else toast('Please drop a JPG, PNG, WEBP or SVG image.', 'error');
  });
...
function initStaticListeners() {
  ...
  document.getElementById('ve-image-dimensions-save')
    ?.addEventListener('click', saveImageDimensions);

  ['ve-image-width-input', 've-image-height-input'].forEach(id => {
    document.getElementById(id)?.addEventListener('input', () => {
      const widthValue = document.getElementById('ve-image-width-input')?.value || '';
      const heightValue = document.getElementById('ve-image-height-input')?.value || '';
      applyImageDimensionsToCanvas(widthValue, heightValue);
    });
  });
